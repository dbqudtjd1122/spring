...
public class RestController {
    ...
    public List<ModelPerson> personlist(Model model
        ...
        if(svr != null) {
            result = svr.getPersonList(person);
        }
        else{
            result = makePersonData(1, 100);
        }
    ...
    public int insertperson(Model model, @RequestBody ModelPerson personJson) {
        ...
        if(svr != null) {
            result = svr.insertPerson(personJson);
        }
        else{
            result = 1;
        }
    ...
    public List<ModelPerson> personfind(Model model
        ...
        if(svr != null) {
            ...
            result = makePersonData(100, 200);
        } 
        
        return result;
    }  

    @RequestMapping(value = "/rest/itemlist", method =  {RequestMethod.GET, RequestMethod.POST} )
    @ResponseBody
    public List<ModelItem> itemlist( Model model
                   , @RequestParam(value="start", required = false, defaultValue = "0") Integer start
                   , @RequestParam(value="end", required = false, defaultValue = "10" ) Integer end
                   ) throws Throwable  {
        logger.info("/rest/articlelist");
