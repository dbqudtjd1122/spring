...
public class RestController {
    ...
    @RequestMapping(value = "/rest/JSONServer", method = RequestMethod.GET)   
    @ResponseBody  
    public Map<String, Object> JSONServer( Model model
                                         , @RequestParam(value="msg", defaultValue = "") String msg ) throws Throwable {
        logger.info("/rest/JSONServer");
    ...
    @RequestMapping(value = "/rest/ContactWS", method = RequestMethod.GET)
    @ResponseBody
    public List<ModelPerson> ContactWS( Model model
                                      , @RequestParam(value="start", required = false, defaultValue = "0") String start
                                      , @RequestParam(value="paging", required = false, defaultValue = "10") String paging
                                      ) throws Throwable {
        logger.info("/rest/ContactWS");
        ...
        if(paging != "" && paging != null) {
            cntPaging = Integer.parseInt(paging);
        }
        
        if(start != "" && start != null) {
            cntStart = Integer.parseInt(start);        
        }
        
        cntLast = cntStart + cntPaging;
        ...
        try {
            ...
            ModelPerson jObject = null;

            for(int i = cntStart ; i<= cntLast; i++) {
                jObject = new ModelPerson();
                jObject.setName ( "name : "  + String.valueOf(i) ); 
                jObject.setId   ( "id : "    + String.valueOf(i) ); 
                jObject.setEmail( "email : " + String.valueOf(i) ); 
                jObject.setPw   ( "pw : "    + String.valueOf(i) );    
    ...
    @RequestMapping(value = "/rest/ModelItem", method = RequestMethod.GET)
    @ResponseBody
    public List<ModelItem> ModelItem( @RequestParam(value="start", required = false, defaultValue = "0") String start
                           , @RequestParam(value="paging", required = false, defaultValue = "10") String paging
                           , Model model) throws Throwable 
    {
        logger.info("/rest/ModelItem");
